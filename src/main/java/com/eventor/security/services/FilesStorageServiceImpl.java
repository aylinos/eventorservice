...
public class FilesStorageServiceImpl implements FilesStorageService {
    private final Path root = Paths.get("profileImages");
    ...
    @Override
    public void save(MultipartFile file, String newFileName) { //Add user id here
        try {
            init();
    ...
    public Stream<Path> loadOne(String fileName) {
        try {
            init();
            Stream<Path> loadedFile = Files.find(this.root, 1, (path, basicFileAttributes) -> {
